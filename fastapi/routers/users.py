...
from services.user_service import UserService  # type: ignore
...
def get_by_id(id: int, db: Session = Depends(get_db)):
    try:
        data = UserService.get_by_id(db, id)
        if not data:
...
@router.post("/")
def create(entity: UserCreate, db: Session = Depends(get_db)):
    try:
        new_id = UserService.create(db, entity)
...
@router.put("/{id}")
@router.patch("/{id}")
def update(id: int, entity: UserUpdate, db: Session = Depends(get_db)):
    try:
        updated = UserService.update(db, id, entity)
        if not updated:
